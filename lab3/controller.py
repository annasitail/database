...
from sqlalchemy.sql import select, and_, update, delete
from model import Student, Teacher, Subject, Subject_Teacher, Schedule, Mark, Search


def run(command):
    engine = create_engine("postgresql://postgres:1234567890@localhost:5432/lab3", echo=True)
    ...
    subjects_teachers = Table('subjects_teachers', meta, autoload=True)

    schedules = Table('schedule', meta, autoload=True)

    marks = Table('marks', meta, autoload=True)

    connection = engine.connect()

    student = Student(connection, students)
    teacher = Teacher(connection, teachers)
    subject = Subject(connection, subjects)
    subject_teacher = Subject_Teacher(connection, subjects_teachers)
    schedule = Schedule(connection, schedules)
    mark = Mark(connection, marks)
    # search = Search(connection, students, teachers, subjects, subjects_teachers, schedules, marks)
    search = Search()
    view = View()
    ...
    if command == "add student":
        ...
        try:
            command_line = int(view.ask_for_values_to_generate("subject"))
        ...
        try:
            command_line = int(view.ask_for_values_to_delete("subjects_teachers record"))
        ...
        try:
            command_line = int(view.ask_for_values_to_generate("subjects_teachers record"))
        ...
        try:
            ...
            v1 = int(command_line.split(", ")[0])
            v2 = command_line.split(", ")[1]
            v3 = command_line.split(", ")[2]
            v4 = int(command_line.split(", ")[3])
            v5 = int(command_line.split(", ")[4])
        ...
        try:
            command_line = int(view.ask_for_values_to_generate("schedule"))
        ...
        try:
            command_line = int(view.ask_for_values_to_generate("mark record"))
